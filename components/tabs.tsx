                ...
                <div className='tab-header flex bg-[#D2D3D4] rounded-t-md text-[#002245]'>
                    {
                        tabHeader?.map((val, i) => (
                            <div key={i} className={
                                `p-2 md:px-11 md:py-3 cursor-pointer ${activeTab === i
                                    ? 'bg-[#002245] text-white rounded-t-md'
                                    : ''}`} onClick={() => setActiveTab(i)}>{val}</div>
                        ))
                    }
                </div>
                {
                    tabs != null &&
                    <div className='tab-content px-5 pt-5 pb-20'>
                        {tabs[activeTab]}
                    </div>
                }
            </div>
